# -*- coding: utf-8 -*-
"""
Created on Wed Aug 23 21:55:09 2017
...
import numpy as np
n=int(input("Enter the degree of the polynomial to be generated: "))
co=[]
for i in range(n+1):
...
if ch==4:
    ...
    p=np.poly1d(c)
    y=p(X)
    plt.figure().suptitle('The Randomly generated polynomial', fontsize=14, fontweight='bold')
    plt.plot(X,y)
    plt.xlabel("x")
    plt.ylabel("y=P(x)")
    plt.show()
    ...
    p1=np.poly1d(c1)
    y1=p1(X)
    plt.figure().suptitle('Integral of the polynomial', fontsize=14, fontweight='bold')
    plt.plot(X,y1)
    plt.xlabel("x")
    plt.ylabel("P1(x)")
    plt.show()
